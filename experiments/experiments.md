...
---
## 2026-07-15 设计一套采集实验室卫星模型，带位姿真值图像的方案
- zed相机配置完成，使用python的sdk
https://www.stereolabs.com/docs/tutorials
- Q1:相机和卫星模型各自如何装 marker、动捕如何同时输出两者的 6DoF?
marker输出的是xyz坐标
1.在卫星正上方和前后左右各贴一个marker，共五个
2.把相机固定在泡沫盒上，在泡沫盒上贴3个marker，成直角三角形。
3.建立动捕全局坐标系（现场标定时定义，原点在地面某角落）
4.计算相机在动捕系下的位姿和卫星在动捕系下的位姿，时间戳对齐
- Q2:相机内参怎么标
1.内参即三维物体到相机像平面的投影矩阵，理想情况下根据焦距的相似三角形即可求得
2.但实际相机由于各种因素，内参矩阵出现缩放、偏移、偏斜等畸变（体现在畸变系数上）。因此需要标定。
3.【相机标定的基本原理与经验分享】https://www.bilibili.com/video/BV1eE411c7kr?vd_source=7b414b12a55fe36688885e68f26d5cdf
其中介绍了
3.1张氏标定法：为优化误差提供较好的初值
3.2棋盘格标定：固定棋盘格，用相机扫一遍。使用OpenCV标定
3.3圆环标定
4.标定一次即可，内参是相机的固定属性

- Q3:动捕刚体坐标系到相机光心坐标系的外参怎么标（手眼标定，这是方案的核心难点）
动捕跟踪的是marker架，我们需要的是光心。两者之间有一个固定的空间偏移,需求出这个偏移
  1. 手眼标定：你的眼睛（相机光心）在头上，但你头上戴的 tracking marker（刚体）不在眼睛的位置。你要知道"眼睛相对于 marker 的确切位置。
  2. 相机marker相对于动捕系的T（已知：由动捕系统得到）
  3. 相机光心相对于标定板的T（已知：由OpenCV计算得）
  4. 动捕系相对于标定板，相机marker和相机光心都是未知且固定的量。通过不同帧联立方程，即可求解。
...
即求卫星相对于光心的T，即transform_matrix
1. 已知：动捕系到光心系（即相机外参）
2. 动捕输出：动捕系到卫星系
3. 以动捕系为世界系，1即c2w，投影在2上即卫星系下的c2w
...
- 实验室卫星采集方案
 ...
 标定动捕系
 标定相机内参
 手眼标定相机外参：即相机marker到光心
 光照保持固定，背景尽量暗且干净
  2. 拍摄流程：
  水平圈（相机与卫星约同高 → 俯角~0°）。绕卫星走一圈，每 10°~15° 拍一张
  中高度圈（相机抬高约 30° 俯角）。再绕一圈
  高角度圈（约 60° 俯角）。再绕一圈
  补充角度（正上方、低角度仰视、侧面特写）
  3. 图像处理：
  Step 1: 时间同步
  在动捕数据流中找与照片最相近的时间戳，必要时线性插值
  Step 2: 计算 c2w
  卫星到光心 = 卫星到动捕 *动捕到相机 *相机到光心
  得到4×4矩阵就是 transform_matrix
  Step 3: 坐标系检查
...
## 2026-07-10 学习Python
- 语法速成
【20分钟学完一遍python基础】https://www.bilibili.com/video/BV1Sz4y1U77N?vd_source=7b414b12a55fe36688885e68f26d5cdf
- 面向对象基础
https://www.bilibili.com/video/BV1sHU9BmEne?spm_id_from=333.788.videopod.episodes&vd_source=69202cdbb3a207af8befc561c94718c2&p=77
- 语法糖
【『教程』Python中常用的九种语法糖】https://www.bilibili.com/video/BV1Nf4y1k7Pu?vd_source=7b414b12a55fe36688885e68f26d5cdf
---
